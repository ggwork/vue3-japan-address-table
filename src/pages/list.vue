  ...
  <div class="main">
    ...
    <div class="table-wrap">
      <el-table :data="tableData" style="width: 100%" border highlight-current-row  @selection-change="tableSelectionChange">
        ...
        <el-table-column label="状态"
          ...
          <template #default="scope">
            <span :class="[scope.row.status ? 'red':'green']">
              {{ statusMap[scope.row.status] || '未下载' }}
            </span>
          </template>
        </el-table-column>
        ...
        <el-form-item>
          <template #default="scope">
            <el-button type="danger" @click="deleteOrder(scope.row)"></el-button>
          </template>
...
function getProductData(orderRowData){
  ...
  orderRowData.proList.forEach((pro,index)=>{
    proList.push([index+1,pro.productName,'','','','','',pro.num,pro.price,pro.totalPrice,''])
  })
  ...
  proList.push(emptyRow)
  // 合计金额和汇率
  proList.push(['合計金額（元）','','','','','','','','',orderRowData.totalMoneyChina],)
  proList.push(['為替レート','','','','','','','','',orderRowData.rate],)
  proList.push(['合計金額（円）','','','','','','','','',orderRowData.totalMoneyJapan],)
...
function deleteOrder(row){
  ElMessageBox.confirm('确定要删除该订单吗？','提示',{
      ...
      await deleteOrderApi({_id:row._id})
      ElMessage({
        type: 'success',
        message: '删除成功',
      })
...
function createPersonExcel(orderRowData){
  ...
  // 顾客id
  worksheet.getCell('C6').value = orderRowData.customerId;
  worksheet.getCell('E6').value = '顧客名：';
  worksheet.getCell('F6').value = orderRowData.customerName;
  worksheet.getCell('K6').value = '発送日：';

  // 添加商品
  let proList = getProductData(orderRowData)
  // 合并商品表格单元格
  // 从第7行开始以下为商品数据
  let  startRowNum = 7
  ...
  worksheet.addRow(['代金受領日：','','',`${orderRowData.getDate}  0:00:00`])
  worksheet.mergeCells(`A${getRowNum}:C${getRowNum}`)

  worksheet.addRow()
  worksheet.addRow(['郵送先：','',` ${orderRowData.getAddress} ${orderRowData.customerPhone} `])
